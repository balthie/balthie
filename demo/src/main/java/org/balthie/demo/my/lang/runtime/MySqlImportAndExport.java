package org.balthie.demo.my.lang.runtime;
...
/**
 * 在进行导出的时候，需要注意命令语句的运行环境，如果已经将mysql安装路径下的bin加入到 系统的path变量中，那么在导出的时候可以直接使用命令语句，否则，就需要在执行命令语句的
...
public class MySqlImportAndExport
{
    ...
    public static void main(String args[]) throws IOException
    {
        InputStream is = MySqlImportAndExport.class.getClassLoader()
                .getResourceAsStream("org/balthie/demo/my/lang/runtime/jdbc.properties");
    ...
    /**
     * 根据属性文件的配置把指定位置的指定文件内容导入到指定的数据库中 在命令窗口进行mysql的数据库导入一般分三步走： 第一步是登到到mysql； mysql -uusername
     * -ppassword -hhost -Pport -DdatabaseName;如果在登录的时候指定了数据库名则会 直接转向该数据库，这样就可以跳过第二步，直接第三步；
     * 第二步是切换到导入的目标数据库；use importDatabaseName； 第三步是开始从目标文件导入数据到目标数据库；source importPath；
     * 
     * @param properties
     * @throws IOException
     */
    ...
    /**
     * 利用属性文件提供的配置来拼装命令语句 在拼装命令语句的时候有一点是需要注意的：一般我们在命令窗口直接使用命令来 进行导出的时候可以简单使用“>”来表示导出到什么地方，即mysqldump
     * -uusername -ppassword databaseName > exportPath， 但在Java中这样写是不行的，它需要你用-r明确的指出导出到什么地方，如：
    ...
    /**
     * 根据属性文件的配置，分三步走获取从目标文件导入数据到目标数据库所需的命令 如果在登录的时候指定了数据库名则会 直接转向该数据库，这样就可以跳过第二步，直接第三步；
     * 
     * @param properties
     * @return
     */
    private static String[] getImportCommand(Properties properties)
    {
        ...
        // 第一步，获取登录命令语句
        String loginCommand = new StringBuffer().append("mysql -u").append(username).append(" -p").append(password).append(" -h")
