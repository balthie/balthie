...
public class CronTriggerExample
{
    ...
    public void run() throws Exception
    {
        ...
        // 创建 group1 同组任务 job2 Job #2 is scheduled to run every other minute starting at 15 seconds past the minute.
        addJob2Scheduler(sched, "job2", "trigger2", "group1", "15 0/2 * * * ?");
        
        // 创建 group1 同组任务 job3 Job #3 is scheduled to run every other minute between 8am and 5pm (17 :00).
        addJob2Scheduler(sched, "job3", "trigger3", "group1", "0 0/2 8-17 * * ?");

        // 创建 group1 同组任务 job4 Job #4 is scheduled to run every three minutes, but only between 5pm and 11pm
        addJob2Scheduler(sched, "job4", "trigger4", "group1", "0 0/3 17-23 * * ?");

        // 创建 group1 同组任务 job5 Job #5 is scheduled to run at 10am on the 1st and 15th days of the month
        addJob2Scheduler(sched, "job5", "trigger5", "group1", "0 0 10am 1,15 * ?");

        // 创建 group1 同组任务 job6 Job #6 is scheduled to run every 30 seconds on weekdays (Monday through Friday)
        addJob2Scheduler(sched, "job6", "trigger6", "group1", "0,30 * * ? * MON-FRI");

        // 创建 group1 同组任务 job7 Job #7 is scheduled to run every 30 seconds on weekends (Saturday and Sunday)
     ...
     */
    public void addJob2Scheduler(Scheduler sched, String jobName, String triggerName, String groupName, String cronExp) throws SchedulerException
    {
        JobDetail job;
        CronTrigger trigger;
        Date ft;
